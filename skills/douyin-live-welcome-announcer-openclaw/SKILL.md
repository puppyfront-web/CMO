...
description: Use when an OpenClaw user wants spoken welcome announcements for viewers joining a Douyin live room on macOS or Windows, especially when monitoring the live room in a browser and needing natural TTS with digit-by-digit nickname reading.
...
This skill packages a self-contained Douyin live-room watcher for OpenClaw users on macOS and Windows. It opens a Playwright-controlled Chromium page, detects join messages such as `某某来了`, and speaks a welcome line with `edge-tts` first and the system local TTS as fallback.

## Use this skill when

- The user is live on Douyin and wants spoken welcome messages for viewers entering the room.
...
- Ask the user for the real live-room URL if the browser is still on `https://live.douyin.com/` homepage content.
...
2. Start the announcer:

```bash
SKILL_DIR="$PWD/skills/douyin-live-welcome-announcer-openclaw"
bash "$SKILL_DIR/scripts/run.sh" "https://live.douyin.com/"
```

3. If the page is not the actual live room, ask the user to open their real Douyin live-room share URL in the controlled browser window.
